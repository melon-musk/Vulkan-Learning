...
struct Vertex {
    ...
    //binding decriptions tells us how to pass data to the gpu
...
const std::vector<uint16_t> indices_indexBuffer = {
    0,1,2,2,3,0
};
...
private:
    ...
    void initVulkan() {
        myWindow.setUserPointer(&framebufferResized);
        createInstance();
        setupDebugMessenger();
        createSurface();
        pickPhysicalDevice();
    ...
    void createTextureImage()
    {
        ...
        stbi_uc* pixels = stbi_load("textures/rohan.jpg", &texWidth, 
            &texHeight, &texChannels, STBI_rgb_alpha);
        VkDeviceSize imageSize = texWidth * texHeight * 4; // casue 4 bytes per pixel
    ...
    {
        VkDeviceSize bufferSize = sizeof(indices_indexBuffer[0]) * indices_indexBuffer.size();
        VkBuffer stagingBuffer;
        VkDeviceMemory stagingBufferMemory;
        ...
        void* deviceMemoryMappingPoint;
        vkMapMemory(logicalDevice, stagingBufferMemory, 0, bufferSize, 0, &deviceMemoryMappingPoint);
        memcpy(deviceMemoryMappingPoint, indices_indexBuffer.data(), (size_t)bufferSize);
        vkUnmapMemory(logicalDevice, stagingBufferMemory);
    ...
    void createVertexBuffer()
    {
        ...
        memcpy(data, vertices.data(), (size_t)bufferSize); // to then from
        vkUnmapMemory(logicalDevice, stagingBufferMemory);
    ...
    void createCommandPool()
    {
        ...
        graphicsPoolInfo.queueFamilyIndex = indices.getGraphicsFamily();

        VkCommandPoolCreateInfo transferPoolInfo{};
        transferPoolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
        transferPoolInfo.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT ;
        transferPoolInfo.queueFamilyIndex = indices.getTransferFamily();
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        //these are probably the instructions that are being recorded
    ...
    void createSwapChain()
    {
        ...
        // checking maxImageCount against zero to see if there is no maximum
        ...
        // swpachain can become invalid or unotimisied for eg during window resize.
    ...
    void createLogicalDevice()
    {
        ...
        std::set<uint32_t> uniqueQueueFamilies = { indices.getGraphicsFamily(), indices.getPresentationFamily(), indices.getTransferFamily()};
        ...
        vkGetDeviceQueue(logicalDevice, indices.getTransferFamily(), 0, &transferQueue);
    }
  ...
  bool isDeviceSuitable(VkPhysicalDevice device)
    ...
    //glfwextenisons required to interface with vulakn
    ...
    void drawFrame()
    {
        ...
        //resetting here because it tells us that we have aquired the frame with no swapchain
        //incompatibility
        ...
        if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || framebufferResized) {
          /*  std::cout << "FRAME RESIED " << framebufferResized << std::endl;*/
